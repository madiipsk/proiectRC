...
public class ServerScadere implements AutoCloseable {

    private DatagramSocket socket;

    public ServerScadere(int port) throws IOException {
        ...
        while (socket != null && !socket.isClosed()) {
            ...
            if (components.length == 3 && components[0].equals("scade")) {
                ...
                if (components[1].equals("8081")){
                    buffer = ("Conectat la portul 8081").getBytes();
                    System.out.println("Client conectat la 8081");
                    DatagramPacket response = new DatagramPacket(buffer, buffer.length, clientAddress, clientPort);
                    socket.send(response);
                } else {
            ...
            } else if (components.length == 3 && components[0].equals("aduna")) {
                //trimite la serverul de scaderi
                request.setAddress(InetAddress.getByName("127.0.0.1"));
                request.setPort(8080);
                socket.send(request);
                System.out.println("am trimis la serverul de adunari");
                //trimite mesaj intermediar clientului
                buffer = "am trimis mai departe la serverul de adunari".getBytes();
                DatagramPacket response = new DatagramPacket(buffer, buffer.length, clientAddress, clientPort);
                socket.send(response);
                //am primit mesajul de la serverul de scaderi
                socket.receive(request);
                System.out.println("am primit de la serverul de adunari");
                ...
                buffer = (Integer.toString(8081) + clientAddress).getBytes();
                DatagramPacket response = new DatagramPacket(buffer, buffer.length, clientAddress, clientPort);
                socket.send(response);
